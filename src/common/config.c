...
// 获取指定参与方的地址
PartyAddress *get_party_address(int party_id) {
  if (party_id >= 0 && party_id <= NUM_PARTIES) {
    return &sys_params.parties[party_id];
  }
  return NULL;
}

// 获取当前参与方ID
int get_current_party_id() { return sys_params.current_party_id; }

// 获取当前参与方的端口
int get_my_port() {
  return sys_params.parties[sys_params.current_party_id].port;
}