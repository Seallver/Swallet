...
const char *message_type_to_str(int type) {
  ...
  case ACK:
    return "ACK";
  case MSG_PUBLIC_VK:
    return "MSG_PUBLIC_VK";
  case MSG_PRESIGN_DATA:
    return "MSG_PRESIGN_DATA";
  case MSG_KEY_EXCHANGE:
    return "MSG_KEY_EXCHANGE";
  case MSG_UV_DATA:
    return "MSG_UV_DATA";
  case REQUEST_SIGN:
    return "REQUEST_SIGN";
...
int broadcast(MessageType type, const char *data) {
  int self_id = sys_params.current_party_id;
  for (int id = 0; id <= NUM_PARTIES; id++) {
    if (id == self_id)
      continue;
    const char *ip = sys_params.parties[id].ip;
    int port = sys_params.parties[id].port;

    send_message(self_id, ip, port, type, data);
  }
